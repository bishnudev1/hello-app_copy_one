...
class ManageVariantsPopDialog extends StatefulWidget {
  const ManageVariantsPopDialog({super.key, required this.productId});

  final String productId;

  @override
  State<ManageVariantsPopDialog> createState() => _ManageVariantsPopDialogState();
}
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  onTap: () async {
                    ...
                    if (res.isNotEmpty) {
                      ...
                      log('My variant value list is: $selectedValue');
                      setState(() {});
                    }
                  },
                ),
                ...
                selectedValue.isNotEmpty
                    ? Row(
                        children: List.generate(selectedValue.length, (subIndex) {
                          log('My variant list is value: ${selectedValue[subIndex]}');
                          return Container(
                            padding: EdgeInsets.all(10),
                            margin: EdgeInsets.only(bottom: 16, right: 10),
                            decoration: BoxDecoration(
    ...
    manageVariantsApiResModel.variants?.forEach((element) {
      if (element.title == variantTitle) {
        element.values?.forEach((elementValue) {
          adminVariantValue.add({"valueId": elementValue.sId, "valueTitle": elementValue.title});
        });
      }
    });
    setState(() {});
                  ...
                  itemBuilder: (context, index) => Column(
                    children: [
                      InkWell(
                        onTap: () {
                          // selectedString = adminCategories[index];
                          selectedString.complete(adminVariantValue[index]['valueTitle']);
                          log('My selected string: ${selectedString.future}');
                          setState(() {});
                          Navigator.pop(context, selectedString);
                        },
                        child: Container(
                          margin: EdgeInsets.only(bottom: 10, top: 10),
                          child: Center(child: Text('${adminVariantValue[index]['valueTitle']}')),
                        ),
                      ),
                      Divider(
                        color: Colors.grey,
  ...
  addVariantValue() async {
    ...
    if (manageVariantsCollectionApiResModel.success == true) {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text(manageVariantsCollectionApiResModel.message.toString())));
    }
  }
}
